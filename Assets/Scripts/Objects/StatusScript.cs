...
public class StatusScript : MonoBehaviour {
    ...
    static public void NewStatus(CharacterScript _owner, CharacterScript _caster, ActionScript _Action)
    {
    ...
    static public void ApplyStatus(CharacterScript _charScript)
    {
        StatusScript[] statScripts = _charScript.GetComponents<StatusScript>();

        for (int i = 1; i < _charScript.m_stats.Length; i++) // Don't do this for the first index since that would reset HP
        {
            _charScript.m_tempStats[i] = _charScript.m_stats[i];

            for (int j = 0; j < statScripts.Length; j++)
                if (statScripts[j].m_lifeSpan > 0)
                    _charScript.m_tempStats[i] += statScripts[j].m_statMod[i];
                else
                    continue;
        }
    }

    static public void UpdateStatus(CharacterScript _charScript, mode _mode)
    {
        StatusScript[] statScripts = _charScript.m_statuses;
        ...
        for (int i = 0; i < statScripts.Length; i++)
        {
            if (statScripts[i].m_mode == _mode && statScripts[i].m_lifeSpan > 0)
            {
                ...
                if (statScripts[i].m_lifeSpan <= 0)
                {
                    ...
                    statScripts[i].DestroyStatus(_charScript, false);
                }
                else
                    PerformEffect(statScripts[i], _charScript);
            }
        }
        ...
        AlignSymbols(_charScript);
    }

    public void DestroyStatus(CharacterScript _charScript, bool _align)
    {
        RemoveEffect();
        Start();
        ApplyStatus(_charScript);

        if (_align)
            AlignSymbols(_charScript.GetComponent<CharacterScript>());
    }
    ...
    static public void DestroyAll(CharacterScript _charScript)
    {
        StatusScript[] statScripts = _charScript.GetComponents<StatusScript>();
    ...
    static public void DestroyAllDebuffs(CharacterScript _charScript)
    {
        StatusScript[] statScripts = _charScript.GetComponents<StatusScript>();
